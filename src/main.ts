...
const extractFromEntry = <K extends keyof DictionaryAPIResponse> (
  ...
  key: K) : DictionaryAPIResponse[K] | undefined => {
...
const clearDefinitionsSection = (): HTMLElement => {
  const definitionsSection = document.getElementById(
    "definitions", //may be null
  ) as HTMLElement; //it worried about returning null, cant define the type
  //use type assertion, when not able to auto infer
...
const createDefinitionItem = (definitionObj: Definition): HTMLElement => { //ensure defobj have structure defined
...
const createPhoneticsSection = ():HTMLElement => {
  const phoneticsSection : HTMLElement = document.getElementById("phonetics") as HTMLElement;
...
const createAudioControl = (): HTMLAudioElement => { //more specific
  const audioControl: HTMLAudioElement = document.createElement("audio");
  audioControl.style.width = "100%";
  // In TypeScript, the style property is a CSSStyleDeclaration object
  //not audioControl.style = "width: 100%" 
